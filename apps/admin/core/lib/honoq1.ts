...
>(
  ...
  Omit<
    UseQueryOptions<TResponse, TError, InferSelectReturnType<TResponse, Options['select']>, QueryKey<T, M, Params>>,
    'queryFn'
  > & {
    queryFn: Exclude<
      UseQueryOptions<
...
// const client = hc('/api')
// const data = useQuery(client.posts, '$get', { query: { filter: 'active' } }, { staleTime: 60000 })
// const data = useQuery(queryOptions(client.posts, '$get', { query: { filter: 'active' } }, { staleTime: 60000 }))
// const data = useQuery(queryOptions(client.posts, '$post', { json: { name: 'john' } }))
