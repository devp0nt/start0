...
// import { Fs0 } from "../../../../modules/lib/fs0/src"
...
// const prepare = async () => {
//   console.log(343434, __dirname)
//   const fs0 = Fs0.create({ cwd: __dirname })
...
//   return { generateEverything, generateRouteFileByPagePath, generateRoutesFile, removeAllUnusedRoutesFiles }
// }

// const init = async () => {
//   const { generateEverything } = await prepare()
//   await generateEverything()
// }

// const process = async (event: string, path: string) => {
//   const { generateRouteFileByPagePath, generateRoutesFile, removeAllUnusedRoutesFiles } = await prepare()
//   if (event !== "delete") {
...
//     init: async () => {
//       // await init({ fs0, _ })
//       execSync(`bun run ${fs0.resolve("gen0.ts")} init`, { stdio: "inherit" })
//     },
//     watchers: {
//       createRouteByPage: {
//         watch: watchGlob,
//         handler: async (ctx, event, path) => {
//           // await process({ fs0, _ }, event, path)
//           execSync(`bun run ${fs0.resolve("gen0.ts")} process ${event} ${path}`, { stdio: "inherit" })
//         },
//       },
//     },
//   }
...
//     if (!event || !path) {
//       console.error("❌ Usage: bun run gen0.ts process <event> <path>")
//     }

//     await process(event, path)
//     console.log(`✅ Processed ${event} event for ${path}`)
//   } else {
//     console.error("❌ Unknown command. Available commands:")
//     console.error("  init    - Initialize and generate everything")
//     console.error("  process - Process file changes (usage: process <event> <path>)")
//   }
// }
