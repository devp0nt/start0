// @gen0:start store.x = await importFromTsFiles({ globPattern: "~/**/route.hono.ts", exportEndsWith: "HonoRoute" })
...
export namespace BackendHonoRouter {
  export const apply = ({ honoApp }: { honoApp: HonoApp.AppType }) => {
    // @gen0:start store.x.exportNames.map(name => print(`${name}({ honoApp })`))
